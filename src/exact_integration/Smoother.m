classdef Smoother < handle
    ...
    methods
        ...
        % constructor method
        function obj = Smoother(initial_time,final_time,time_step,sample_times,observations)
        ...
        function backward_dist = get_backward(obj)
         ...
         % get the backward distribution into single matrix
        function smoothed_dist = get_smoothed(obj)
            % get distribution
        ...
        function forward_filter(obj,forward_backward,initial_dist)
            ...
            % observations
            for i = 1:obj.num_intervals
                [~,dist] = forward_backward.integrate(initial_dist,obj.time_grid{i});
        ...
        function backward_filter(obj,forward_backward)
            ...
            for i = (obj.num_intervals-1):-1:1
                [~,dist] = forward_backward.backward_integrate(terminal_dist,obj.time_grid{i}(end:-1:1));
                obj.backward_dist{i} = dist(:,end:-1:1);
                if i > 1
                    terminal_dist = log(abs(dist(:,end)))+obj.observation_likelihood(:,i-1)-obj.filter_norm(i-1);
