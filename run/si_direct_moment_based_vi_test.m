...
% this version uses a specialized implementation of the system rather than
% a general ctbn structure
...
initial = x;
initial_rates = repmat(rates,[length(x),1]);
model = SIS_Direct(initial,initial_rates,A,[system.t_min,system.t_max],2);

% set up observation model
num_species = 3;
observed_species = 3;
obs_model = GaussObsBernoulli(sigma,model.num_nodes,'reduced');

% set up options
options = struct;
options.operation_mode = 'smoothing';
options.encoding = 'reduced';
options.control_bounds = [0,1e6];
...
measurement = simulate_measurement([system.t_min,times],[system.initial,states],noise,obs_times,selection);
...
alpha = [initial_rates,initial_rates]';
...
for i = 1:6
    ...
    plot(obs_times,measurement(cnt,:),'xk','linewidth',2)
    cnt = cnt+1;
    end
    plot(t_vi,states_vi(:,i),'-b','linewidth',2)
